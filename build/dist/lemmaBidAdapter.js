...
var spec = {
  ...
  isBidRequestValid: function isBidRequestValid(bid) {
    if (bid && bid.params) {
      if (!__WEBPACK_IMPORTED_MODULE_0__src_utils__["isNumber"](bid.params.pubId)) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"](LOG_WARN_PREFIX + 'Error: publisherId is mandatory and cannot be string. Call to OpenBid will not be sent for ad unit: ' + JSON.stringify(bid));
        return false;
      }

      if (!bid.params.adunitId) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"](LOG_WARN_PREFIX + 'Error: adUnitId is mandatory. Call to OpenBid will not be sent for ad unit: ' + JSON.stringify(bid));
        return false;
      } // video ad validation
...
function parseRTBResponse(request, response) {
  ...
  try {
    ...
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logError"](LOG_WARN_PREFIX, 'ERROR ', error);
  }
...
function oRTBTemplate(bidRequests, conf) {
  try {
    ...
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logError"](LOG_WARN_PREFIX, 'ERROR ', ex);
  }
}
...
function _getSizes(request) {
  if (request.sizes && __WEBPACK_IMPORTED_MODULE_0__src_utils__["isArray"](request.sizes[0]) && request.sizes[0].length > 0) {
...
function _getBannerRequest(bid) {
  ...
  if (bid.mediaType === 'banner' || __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'mediaTypes.banner')) {
    ...
    if (sizes && sizes.length > 0) {
      ...
      if (bannerData) {
        bObj = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepClone"](bannerData);
      }
      ...
      __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"](LOG_WARN_PREFIX + 'Error: mediaTypes.banner.sizes missing for adunit: ' + bid.params.adunitId);
    }
  }
...
function _getVideoRequest(bid) {
  ...
  if (bid.mediaType === 'video' || __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'mediaTypes.video')) {
    var params = bid ? bid.params : null;
    var sizes = _getSizes(bid) || [];
    ...
    if (sizes && sizes.length > 0) {
      ...
      if (videoData) {
        vObj = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepClone"](videoData);
      }

      vObj.w = sizes[0];
      vObj.h = sizes[1];
    } else {
      __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"](LOG_WARN_PREFIX + 'Error: mediaTypes.video.sizes missing for adunit: ' + bid.params.adunitId);
    }
  }
...
function _getImpressionObject(bid) {
  ...
  return impression.hasOwnProperty(__WEBPACK_IMPORTED_MODULE_2__src_mediaTypes__["b" /* BANNER */]) || impression.hasOwnProperty(__WEBPACK_IMPORTED_MODULE_2__src_mediaTypes__["d" /* VIDEO */]) ? impression : undefined;
}
