  ...
  isBidRequestValid: function isBidRequestValid(bid) {
    ...
    if (valid && bid.params.hasOwnProperty('currency')) {
      if (SUPPORTED_CURRENCIES.indexOf(bid.params.currency) === -1) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils__["logError"]('Invalid currency type, we support only JPY and USD!');
        valid = false;
      }
    }
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    ...
    validBidRequests.forEach(function (bid, i) {
      ...
      serverRequests.push({
        method: 'GET',
        url: endpoint,
        data: __WEBPACK_IMPORTED_MODULE_0__src_utils__["parseQueryStringParameters"](data)
      });
    });
