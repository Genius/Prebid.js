...
var spec = {
  ...
  interpretResponse: function interpretResponse(serverResponse, request) {
    ...
    } catch (err) {// json error initial request can't be read
    } // For this adapter, serverResponse is a list


    serverResponse.body.forEach(function (response) {
      var bid = createBid(response, bidRequests);
...
function getHostname(bidderRequest) {
  var dcHostname = __WEBPACK_IMPORTED_MODULE_2_core_js_pure_features_array_find_js___default()(bidderRequest, function (bid) {
...
function createEndpoint(bidRequests, bidderRequest) {
  ...
  return __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["buildUrl"]({
    protocol: 'https',
...
function getSizeArray(bid) {
  ...
  return __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["parseSizesInput"](inputSize);
}
