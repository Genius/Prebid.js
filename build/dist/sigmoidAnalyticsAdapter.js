...
function isSessionIdTimeoutExpired() {
  var cpmSessionTimestamp = storage.getDataFromLocalStorage(buildSessionIdTimeoutLocalStorageKey());
...
function isUtmTimeoutExpired() {
  var utmTimestamp = storage.getDataFromLocalStorage(buildUtmLocalStorageTimeoutKey());
...
function buildBidWon(eventType, args) {
  ...
  if (checkAdUnitConfig()) {
    if (__WEBPACK_IMPORTED_MODULE_0_core_js_pure_features_array_includes_js___default()(initOptions.adUnits, args.adUnitCode)) {
      bidWon.events = [{
        args: args,
        eventType: eventType
      }];
...
function filterBidsByAdUnit(bids) {
  ...
  bids.forEach(function (bid) {
    if (__WEBPACK_IMPORTED_MODULE_0_core_js_pure_features_array_includes_js___default()(initOptions.adUnits, bid.placementCode)) {
...
function isValidEvent(eventType, adUnitCode) {
  if (checkAdUnitConfig()) {
    ...
    if (!__WEBPACK_IMPORTED_MODULE_0_core_js_pure_features_array_includes_js___default()(initOptions.adUnits, adUnitCode) && __WEBPACK_IMPORTED_MODULE_0_core_js_pure_features_array_includes_js___default()(validationEvents, eventType)) {
      return false;
    }
  }
...
var sigmoidAdapter = _extends(Object(__WEBPACK_IMPORTED_MODULE_1__src_AnalyticsAdapter_js__["a" /* default */])({
  url: url,
...
function send(eventType, data, sendDataType) {
  ...
  AWS.config.region = 'us-east-1'; // eslint-disable-next-line no-undef
