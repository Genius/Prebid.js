...
var SUPPORTED_AD_TYPES = [__WEBPACK_IMPORTED_MODULE_3__src_mediaTypes_js__["b" /* BANNER */], __WEBPACK_IMPORTED_MODULE_3__src_mediaTypes_js__["d" /* VIDEO */], __WEBPACK_IMPORTED_MODULE_3__src_mediaTypes_js__["c" /* NATIVE */]];
var SOURCE = 'pbjs10';
...
var BIDADAPTERVERSION = 'r20191128PB30';
var spec = {
  code: BIDDER_CODE,
  ...
  isBidRequestValid: function isBidRequestValid(bid) {
    if (!bid || !bid.params) {
      __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logWarn"](BIDDER_CODE + ': Missing bid parameters');
      return false;
    }

    if (!__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](bid.params.adzoneid)) {
      __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logWarn"](BIDDER_CODE + ': adzoneid must be specified as a string');
      return false;
    }
    ...
    if (isVideoRequest(bid)) {
      if (!bid.params.video.mimes) {
        ...
        __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logWarn"](BIDDER_CODE + ': mimes should be specified for videos');
      } else if (!__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](bid.params.video.mimes) || !bid.params.video.mimes.every(function (s) {
        return __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](s);
      })) {
        __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logWarn"](BIDDER_CODE + ': mimes must be an array of strings');
        return false;
      }
      ...
      if (context !== 'instream') {
        __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logWarn"](BIDDER_CODE + ': video context must be valid - instream');
        return false;
      }
    }
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logMessage"]("buildRequests: ".concat(JSON.stringify(validBidRequests)));
    var dt = new Date();
    ...
    var bt = __WEBPACK_IMPORTED_MODULE_0__src_config_js__["b" /* config */].getConfig('bidderTimeout');
    ...
    var beaconParams = {
      ...
      dpr: ratio,
      bt: bt,
      isinframe: __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["inIframe"](),
      cookies: __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["checkCookieSupport"]() ? '1' : '0',
      tz: dt.getTimezoneOffset(),
      dt: __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["timestamp"](),
      iob: iobavailable ? '1' : '0',
      pbjs: "4.2.0",
      rndid: Math.floor(Math.random() * (999999 - 100000 + 1)) + 100000
    };
    var referrer = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bidderRequest, 'refererInfo.referer');
    var page = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bidderRequest, 'refererInfo.canonicalUrl') || __WEBPACK_IMPORTED_MODULE_0__src_config_js__["b" /* config */].getConfig('pageUrl') || __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](window, 'location.href');
    ...
    var biddercustom = __WEBPACK_IMPORTED_MODULE_0__src_config_js__["b" /* config */].getConfig(BIDDER_CODE);

    if (biddercustom) {
      Object.keys(biddercustom).filter(function (param) {
        return __WEBPACK_IMPORTED_MODULE_4_core_js_pure_features_array_includes_js___default()(USER_PARAMS_AUCTION, param);
    ...
    validBidRequests.forEach(function (bid, index) {
      adZoneIds.push(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["getBidIdParameter"]('adzoneid', bid.params));
      prebidBidIds.push(bid.bidId);

      if (isBannerRequest(bid)) {
        sizes.push(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["parseSizesInput"](bid.mediaTypes.banner.sizes).join('|'));
      }
      ...
      var bidfloor = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["getBidIdParameter"]('bidfloor', bid.params) || 0;
      ...
      if (isVideoRequest(bid)) {
        ...
        beaconParams['video.context' + '.' + index] = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'mediaTypes.video.context');
        sizes.push(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["parseSizesInput"](bid.mediaTypes.video.playerSize).join('|'));
      } // copy all custom parameters impression level parameters not supported above


      var customBidParams = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["getBidIdParameter"]('custom', bid.params) || {};

      if (customBidParams) {
        Object.keys(customBidParams).filter(function (param) {
          return __WEBPACK_IMPORTED_MODULE_4_core_js_pure_features_array_includes_js___default()(USER_PARAMS_BID, param);
        }).forEach(function (param) {
    ...
    if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](validBidRequests, '0.userId.pubcid'))) {
      beaconParams.pubcid = validBidRequests[0].userId.pubcid;
    }

    if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](validBidRequests, '0.userId.tdid'))) {
      beaconParams.tdid = validBidRequests[0].userId.tdid;
    }

    if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](validBidRequests, '0.userId.id5id'))) {
      beaconParams.id5id = validBidRequests[0].userId.id5id;
    }

    if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](validBidRequests, '0.userId.idl_env'))) {
      beaconParams.idl_env = validBidRequests[0].userId.idl_env;
    }

    var adxcgRequestUrl = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["buildUrl"]({
      protocol: 'https',
  ...
  interpretResponse: function interpretResponse(serverResponse, bidRequests) {
    ...
    if (serverResponse) {
      serverResponse.forEach(function (serverResponseOneItem) {
        ...
        __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logMessage"]("submitting bid[".concat(serverResponseOneItem.bidId, "]: ").concat(JSON.stringify(bid)));
        bids.push(bid);
      });
    } else {
      __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logMessage"]("empty bid response");
    }
...
function isVideoRequest(bid) {
  return bid.mediaType === 'video' || !!__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'mediaTypes.video');
}

function isBannerRequest(bid) {
  return bid.mediaType === 'banner' || !!__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'mediaTypes.banner');
}

function isNativeRequest(bid) {
  return bid.mediaType === 'native' || !!__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'mediaTypes.native');
}
