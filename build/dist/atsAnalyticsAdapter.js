...
/* harmony export (immutable) */ __webpack_exports__["checkUserBrowser"] = checkUserBrowser;
/* harmony export (immutable) */ __webpack_exports__["browserIsFirefox"] = browserIsFirefox;
/* harmony export (immutable) */ __webpack_exports__["browserIsIE"] = browserIsIE;
/* harmony export (immutable) */ __webpack_exports__["browserIsEdge"] = browserIsEdge;
/* harmony export (immutable) */ __webpack_exports__["browserIsChrome"] = browserIsChrome;
/* harmony export (immutable) */ __webpack_exports__["browserIsSafari"] = browserIsSafari;
...
function checkUserBrowser() {
  var firefox = browserIsFirefox();
  var chrome = browserIsChrome();
  var edge = browserIsEdge();
  var safari = browserIsSafari();
...
function callHandler(evtype, args) {
  if (evtype === __WEBPACK_IMPORTED_MODULE_1__src_constants_json___default.a.EVENTS.BID_REQUESTED) {
    handlerRequest = handlerRequest.concat(bidRequestedHandler(args));
  } else if (evtype === __WEBPACK_IMPORTED_MODULE_1__src_constants_json___default.a.EVENTS.BID_RESPONSE) {
    handlerResponse.push(bidResponseHandler(args));
  }

  if (evtype === __WEBPACK_IMPORTED_MODULE_1__src_constants_json___default.a.EVENTS.AUCTION_END) {
    if (handlerRequest.length) {
      var events = [];

      if (handlerResponse.length) {
        events = handlerRequest.filter(function (request) {
          return handlerResponse.filter(function (response) {
            if (request.bid_id === response.bid_id) {
              _extends(request, response);
            }
          });
        });
      } else {
...
var atsAnalyticsAdapter = _extends(Object(__WEBPACK_IMPORTED_MODULE_0__src_AnalyticsAdapter_js__["a" /* default */])({
  host: host,
  ...
  track: function track(_ref) {
    ...
    if (eventType === __WEBPACK_IMPORTED_MODULE_1__src_constants_json___default.a.EVENTS.AUCTION_END) {
      ...
      try {
        var dataToSend = {
          'Data': atsAnalyticsAdapter.context.events
        };
        var strJSON = JSON.stringify(dataToSend);
        Object(__WEBPACK_IMPORTED_MODULE_4__src_ajax_js__["a" /* ajax */])(atsAnalyticsAdapter.context.host, function () {}, strJSON, {
          method: 'POST',
...
atsAnalyticsAdapter.originEnableAnalytics = atsAnalyticsAdapter.enableAnalytics; // override enableAnalytics so we can get access to the config passed in from the page

atsAnalyticsAdapter.enableAnalytics = function (config) {
  if (!config.options.pid) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('Publisher ID (pid) option is not defined. Analytics won\'t work');
    return;
  }

  if (!config.options.host) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('Host option is not defined. Analytics won\'t work');
    return;
  }
...
/* harmony default export */ __webpack_exports__["default"] = (atsAnalyticsAdapter);
