...
var spec = {
  ...
  interpretResponse: function interpretResponse(serverResponse, bidRequest) {
    ...
    for (var i = 0; i < responses.length; i++) {
      data = responses[i];
      type = data.response === 'banner' ? __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes__["b" /* BANNER */] : __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes__["d" /* VIDEO */];
      bid = bids[i];

      if (VALID_RESPONSES[data.response] && (verifySize(data, bid.sizes) || type === __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes__["d" /* VIDEO */])) {
        bidObject = {
          requestId: bid.bidId,
