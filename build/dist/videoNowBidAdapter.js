...
function buildRequests(validBidRequests, bidderRequest) {
  __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]("".concat(BIDDER_CODE, ". buildRequests"));
  var requests = [];
...
function createResponseBid(bidInfo, bidId, cur, placementId) {
  ...
  if (!initPath) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logError"]("vnInitModulePath is not defined");
    return null;
  }
  ...
  if (!min && !log) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logError"]('module\'s paths are not defined');
    return null;
  }
    ...
    renderer: {
      ...
      render: function render() {
        ...
        if (el) {
          ...
          Object(__WEBPACK_IMPORTED_MODULE_3__src_adloader_js__["a" /* loadExternalScript */])("".concat(initPath).concat(~initPath.indexOf('?') ? '&' : '?', "profileId=").concat(pId), 'outstream');
        } else {
          __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logError"]("bidAdapter ".concat(BIDDER_CODE, ": ").concat(placementId, " not found"));
        }
      }
    }
  };
...
function getUserSyncs(syncOptions, serverResponses) {
  ...
  __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]("".concat(BIDDER_CODE, " getUserSyncs() syncs=").concat(syncs.length));
  return syncs;
}
...
function onBidWon(bid) {
  ...
  if (nurl) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["triggerPixel"](__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["replaceAuctionPrice"](nurl, bid.cpm));
  }
}
