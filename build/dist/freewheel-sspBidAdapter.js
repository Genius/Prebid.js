...
function getPricing(xmlNode) {
  ...
  if (pricingExtNode) {
    ...
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"]('PREBID - ' + BIDDER_CODE + ': No bid received or missing pricing extension.');
  }
...
var spec = {
  ...
  */
  buildRequests: function buildRequests(bidRequests, bidderRequest) {
    ...
    var buildRequest = function buildRequest(currentBidRequest, bidderRequest) {
      ...
      if (currentBidRequest.mediaTypes.video && currentBidRequest.mediaTypes.video.playerSize) {
        ...
        if (__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isArray"](currentBidRequest.mediaTypes.video.playerSize[0])) {
  ...
  */
  interpretResponse: function interpretResponse(serverResponse, request) {
    ...
    if (bidrequest.mediaTypes.video && bidrequest.mediaTypes.video.playerSize) {
      // If mediaTypes is video, get size from mediaTypes.video.playerSize per http://prebid.org/blog/pbjs-3
      if (__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isArray"](bidrequest.mediaTypes.video.playerSize[0])) {
    ...
    try {
      ...
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"]('Prebid.js - ' + BIDDER_CODE + ' : ' + err);
      return;
    }

    var princingData = getPricing(xmlDoc);
    var creativeId = getCreativeId(xmlDoc);
    var dealId = getDealId(xmlDoc);
