...
var realvuAnalyticsAdapter = Object(__WEBPACK_IMPORTED_MODULE_0__src_AnalyticsAdapter_js__["a" /* default */])({
...
var lib = {
  ...
  tru: function tru(a, f) {
    var pin = a.pins[0];
    var s2 = 'https://ac.realvu.net/flip/3/c=' + pin.partner_id + '_f=' + f + '_r=' + a.riff + '_s=' + a.w + 'x' + a.h;
    if (a.p) s2 += '_p=' + a.p;
  ...
  readPos: function readPos(a) {
    try {
      var s = storage.getDataFromLocalStorage(this.keyPos(a));
  ...
  incrMem: function incrMem(a, evt, name) {
    try {
      ...
      var vmem = storage.getDataFromLocalStorage(k1);
  ...
  score: function score(a, name) {
    try {
      var vstr = storage.getDataFromLocalStorage(this.keyPos(a) + '.' + name);
