...
var ENDPOINT_DEV = 'https://dev.server.cpmstar.com/view.aspx';
var ENDPOINT_STAGING = 'https://staging.server.cpmstar.com/view.aspx';
var ENDPOINT_PRODUCTION = 'https://server.cpmstar.com/view.aspx';
var DEFAULT_TTL = 300;
...
var spec = {
  ...
  getMediaType: function getMediaType(bidRequest) {
    if (bidRequest == null) return __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["b" /* BANNER */];
    return !__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["deepAccess"](bidRequest, 'mediaTypes.video') ? __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["b" /* BANNER */] : __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */];
  },
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    ...
    for (var i = 0; i < validBidRequests.length; i++) {
      ...
      var e = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('endpoint', bidRequest.params);
      ...
      var url = ENDPOINT + '?media=' + mediaType + (mediaType == __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */] ? videoArgs : '') + '&json=c_b&mv=1&poolid=' + __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('placementId', bidRequest.params) + '&reachedTop=' + encodeURIComponent(bidderRequest.refererInfo.reachedTop) + '&requestid=' + bidRequest.bidId + '&referer=' + encodeURIComponent(referer);
      ...
      if (__WEBPACK_IMPORTED_MODULE_3__src_config_js__["b" /* config */].getConfig('coppa')) {
        url += '&tfcd=' + (__WEBPACK_IMPORTED_MODULE_3__src_config_js__["b" /* config */].getConfig('coppa') ? 1 : 0);
      }
  ...
  interpretResponse: function interpretResponse(serverResponse, request) {
    ...
    for (var i = 0; i < serverResponse.body.length; i++) {
      ...
      if (!rawBid) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"]('cpmstarBidAdapter: server response failed check');
        return;
      }
      ...
      if (!cpm) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"]('cpmstarBidAdapter: server response failed check. Missing cpm');
        return;
      }
      ...
      if (mediaType == __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["b" /* BANNER */] && rawBid.code) {
        bidResponse.ad = rawBid.code + (rawBid.px_cr ? "\n<img width=0 height=0 src='" + rawBid.px_cr + "' />" : '');
      } else if (mediaType == __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */] && rawBid.creativemacros && rawBid.creativemacros.HTML5VID_VASTSTRING) {
        var playerSize = spec.getPlayerSize(bidRequest);
        ...
        return __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logError"]('bad response', rawBid);
      }
