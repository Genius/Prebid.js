      ...
      for (var i = 0; i < validBids.length; i += 1) {
        if (typeof validBids[i].params.clientID !== 'undefined') {
          if (validBids[i].adUnitCode in scores) {
            var deviceDealId = Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(scores, "device.".concat(validBids[i].params.device, ".").concat(validBids[i].adUnitCode));
