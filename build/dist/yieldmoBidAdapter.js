...
var YieldmoAdapter = function YieldmoAdapter() {
  ...
  function _registerPlacementBid(response) {
    ...
    if (response && response.cpm && response.cpm !== 0) {
      ...
      // no response data
      if (bidObj) {
        utils.logMessage('No prebid response from yieldmo for placementCode: ' + bidObj.placementCode);
      }
  ...
  function _registerNoResponseBids() {
    var yieldmoBidRequests = pbjs._bidsRequested.find((function (bid) {
      return bid.bidderCode === 'yieldmo';
    }));
