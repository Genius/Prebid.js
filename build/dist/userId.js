...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__src_adapterManager_js__ = __webpack_require__(7);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__src_hook_js__ = __webpack_require__(13);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__eids_js__ = __webpack_require__(44);
...
var coreStorage = Object(__WEBPACK_IMPORTED_MODULE_9__src_storageManager_js__["a" /* getCoreStorageManager */])('userid');
/** @type {string[]} */
...
function setStoredValue(submodule, value) {
  ...
  try {
    var valueStr = __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isPlainObject"](value) ? JSON.stringify(value) : value;
    ...
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"](error);
  }
}
/**
...
function getStoredValue(storage) {
  ...
  try {
    ...
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"](e);
  }
...
function hasGDPRConsent(consentData) {
  if (consentData && typeof consentData.gdprApplies === 'boolean' && consentData.gdprApplies) {
    ...
    if (consentData.apiVersion === 1 && __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["deepAccess"](consentData, 'vendorData.purposeConsents.1') === false) {
      return false;
    }

    if (consentData.apiVersion === 2 && __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["deepAccess"](consentData, 'vendorData.purpose.consents.1') === false) {
      return false;
    }
  }
...
function processSubmoduleCallbacks(submodules, cb) {
  var done = cb ? __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["delayExecution"](cb, submodules.length) : function () {};
  submodules.forEach(function (submodule) {
    submodule.callback(function callbackCompleted(idObj) {
      ...
      if (idObj) {
        ...
        __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logInfo"]("".concat(MODULE_NAME, ": ").concat(submodule.submodule.name, " - request id responded with an empty value"));
      }
...
function getCombinedSubmoduleIds(submodules) {
  ...
  var combinedSubmoduleIds = submodules.filter(function (i) {
    return __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isPlainObject"](i.idObj) && Object.keys(i.idObj).length;
...
function addIdDataToAdUnitBids(adUnits, submodules) {
  ...
  var combinedSubmoduleIdsAsEids = Object(__WEBPACK_IMPORTED_MODULE_8__eids_js__["a" /* createEidsArray */])(combinedSubmoduleIds);
...
function requestBidsHook(fn, reqBidsConfigObj) {
  ...
  initializeSubmodulesAndExecuteCallbacks(function () {
    ...
    addIdDataToAdUnitBids(reqBidsConfigObj.adUnits || Object(__WEBPACK_IMPORTED_MODULE_4__src_prebidGlobal_js__["a" /* getGlobal */])().adUnits, initializedSubmodules); // calling fn allows prebid to continue processing
...
function getUserIdsAsEids() {
  // initialize submodules only when undefined
  initializeSubmodulesAndExecuteCallbacks();
  return Object(__WEBPACK_IMPORTED_MODULE_8__eids_js__["a" /* createEidsArray */])(getCombinedSubmoduleIds(initializedSubmodules));
}
/**
...
var validateGdprEnforcement = Object(__WEBPACK_IMPORTED_MODULE_7__src_hook_js__["b" /* hook */])('sync', function (submodules, consentData) {
  return {
...
function initSubmodules(submodules, consentData) {
  ...
  if (!hasValidated && !hasGDPRConsent(consentData)) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logWarn"]("".concat(MODULE_NAME, " - gdpr permission not valid for local storage or cookies, exit module"));
    return [];
  }
  ...
  return userIdModules.reduce(function (carry, submodule) {
    ...
    if (submodule.config.storage) {
      ...
      if (__WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isPlainObject"](response)) {
        if (response.id) {
      ...
      if (__WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isPlainObject"](_response)) {
        if (typeof _response.callback === 'function') {
          submodule.callback = _response.callback;
        }

        if (_response.id) {
          submodule.idObj = submodule.submodule.decode(_response.id, submodule.config.params);
        }
      }
    }
...
function getValidSubmoduleConfigs(configRegistry, submoduleRegistry, activeStorageTypes) {
  ...
  return configRegistry.reduce(function (carry, config) {
    ...
    if (!config || __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isEmptyStr"](config.name)) {
      return carry;
    ...
    if (config.storage && !__WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isEmptyStr"](config.storage.type) && !__WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isEmptyStr"](config.storage.name) && activeStorageTypes.indexOf(config.storage.type) !== -1) {
      carry.push(config);
    } else if (__WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isPlainObject"](config.value)) {
      carry.push(config);
...
function updateSubmodules() {
  ...
  if (!addedUserIdHook && submodules.length) {
    ...
    Object(__WEBPACK_IMPORTED_MODULE_4__src_prebidGlobal_js__["a" /* getGlobal */])().requestBids.before(requestBidsHook, 40);
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logInfo"]("".concat(MODULE_NAME, " - usersync config updated for ").concat(submodules.length, " submodules"));
...
function init(config) {
  ...
  if (validStorageTypes.indexOf(COOKIE) !== -1 && (coreStorage.getCookie('_pbjs_id_optout') || coreStorage.getCookie('_pubcid_optout'))) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logInfo"]("".concat(MODULE_NAME, " - opt-out cookie found, exit module"));
    return;
  ...
  if (validStorageTypes.indexOf(LOCAL_STORAGE) !== -1 && (coreStorage.getDataFromLocalStorage('_pbjs_id_optout') || coreStorage.getDataFromLocalStorage('_pubcid_optout'))) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logInfo"]("".concat(MODULE_NAME, " - opt-out localStorage found, exit module"));
    return;
  ...
  config.getConfig(function (conf) {
    ...
    if (userSync && userSync.userIds) {
      ...
      syncDelay = __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isNumber"](userSync.syncDelay) ? userSync.syncDelay : DEFAULT_SYNC_DELAY;
      auctionDelay = __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["isNumber"](userSync.auctionDelay) ? userSync.auctionDelay : NO_AUCTION_DELAY;
      updateSubmodules();
    }
  }); // exposing getUserIds function in global-name-space so that userIds stored in Prebid can be used by external codes.

  Object(__WEBPACK_IMPORTED_MODULE_4__src_prebidGlobal_js__["a" /* getGlobal */])().getUserIds = getUserIds;
  Object(__WEBPACK_IMPORTED_MODULE_4__src_prebidGlobal_js__["a" /* getGlobal */])().getUserIdsAsEids = getUserIdsAsEids;
} // init config update listener to start the application

init(__WEBPACK_IMPORTED_MODULE_1__src_config_js__["b" /* config */]);
Object(__WEBPACK_IMPORTED_MODULE_7__src_hook_js__["c" /* module */])('userId', attachIdSystem);
