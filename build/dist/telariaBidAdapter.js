...
var spec = {
  ...
  interpretResponse: function interpretResponse(serverResponse, bidderRequest) {
    ...
    } else if (!__WEBPACK_IMPORTED_MODULE_0__src_utils__["isEmpty"](bidResult.seatbid)) {
      bidResult.seatbid[0].bid.forEach(function (tag) {
        bids.push(createBid(__WEBPACK_IMPORTED_MODULE_4__src_constants__["STATUS"].GOOD, bidderRequest, tag, width, height, BIDDER_CODE));
      });
    }
  ...
  getUserSyncs: function getUserSyncs(syncOptions, serverResponses) {
    ...
    if (syncOptions.pixelEnabled && serverResponses.length) {
      (__WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](serverResponses, '0.body.ext.telaria.userSync') || []).forEach(function (url) {
        return syncs.push({
          type: 'image',
          url: url
        });
  ...
  onTimeout: function onTimeout(timeoutData) {
    ...
    if (url) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils__["triggerPixel"](url);
    }
  }
};
...
function getEncodedValIfNotEmpty(val) {
  return !__WEBPACK_IMPORTED_MODULE_0__src_utils__["isEmpty"](val) ? encodeURIComponent(val) : '';
}
/**
...
function getSupplyChainAsUrlParam(schainObject) {
  if (__WEBPACK_IMPORTED_MODULE_0__src_utils__["isEmpty"](schainObject)) {
    return '';
  }
...
function getUrlParams(params, schainFromBidRequest) {
  ...
  if (!__WEBPACK_IMPORTED_MODULE_0__src_utils__["isEmpty"](params)) {
    for (var key in params) {
      if (key !== 'schain' && params.hasOwnProperty(key) && !__WEBPACK_IMPORTED_MODULE_0__src_utils__["isEmpty"](params[key])) {
        urlSuffix += "&".concat(key, "=").concat(params[key]);
      }
    }

    urlSuffix += getSupplyChainAsUrlParam(!__WEBPACK_IMPORTED_MODULE_0__src_utils__["isEmpty"](schainFromBidRequest) ? schainFromBidRequest : params['schain']);
  }
...
var getTimeoutUrl = function getTimeoutUrl(timeoutData) {
  var params = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](timeoutData, '0.params.0');

  if (!__WEBPACK_IMPORTED_MODULE_0__src_utils__["isEmpty"](params)) {
    var url = "".concat(getScheme()).concat(EVENTS_ENDPOINT);
    url += "?srcPageUrl=".concat(getSrcPageUrl(params));
...
function generateUrl(bid, bidderRequest) {
  ...
  var supplyCode = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'params.supplyCode');
  var adCode = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'params.adCode');

  if (supplyCode && adCode) {
    var url = "".concat(getScheme()).concat(supplyCode, ".").concat(TAG_ENDPOINT, "?adCode=").concat(adCode);
    ...
    url += "".concat(getUrlParams(bid.params, bid.schain));
    url += "&srcPageUrl=".concat(getSrcPageUrl(bid.params));
