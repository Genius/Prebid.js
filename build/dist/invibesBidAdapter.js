...
var storage = Object(__WEBPACK_IMPORTED_MODULE_2__src_storageManager_js__["b" /* getStorageManager */])(CONSTANTS.INVIBES_VENDOR_ID);
var spec = {
...
Object(__WEBPACK_IMPORTED_MODULE_1__src_adapters_bidderFactory_js__["registerBidder"])(spec); // some state info is required: cookie info, unique user visit id
...
function isBidRequestValid(bid) {
  if (invibes && _typeof(invibes.bidResponse) === 'object') {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - Bid response already received. Invibes only responds to one bid request per user visit');
    return false;
  }
...
function handleResponse(responseObj, bidRequests) {
  if (bidRequests == null || bidRequests.length === 0) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - No bids have been requested');
    return [];
  }

  if (!responseObj) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - Bid response is empty');
    return [];
  }
  ...
  if (_typeof(bidModel) !== 'object') {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - Bidding is not configured');
    return [];
  }
  ...
  if (!Array.isArray(ads) || ads.length < 1) {
    if (responseObj.AdReason != null) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - ' + responseObj.AdReason);
    }

    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - No ads available');
    return [];
  }
  ...
  if (bidModel.PlacementId == null) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - No Placement Id in response');
    return [];
  }
  ...
  for (var i = 0; i < bidRequests.length; i++) {
    ...
    if (bidModel.PlacementId == bidRequest.params.placementId) {
      ...
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Invibes Adapter - Incorrect Placement Id: ' + bidRequest.params.placementId);
    }
  }
...
function readGdprConsent(gdprConsent) {
  if (gdprConsent && gdprConsent.vendorData) {
    ...
    var purposeConsents = getPurposeConsents(gdprConsent.vendorData);

    if (purposeConsents == null) {
      return 0;
    }

    var properties = Object.keys(purposeConsents);
    var purposeConsentsCounter = getPurposeConsentsCounter(gdprConsent.vendorData);

    if (properties.length < purposeConsentsCounter) {
      return 0;
    }

    for (var i = 0; i < purposeConsentsCounter; i++) {
      if (!purposeConsents[properties[i]] || purposeConsents[properties[i]] === 'false') {
        return 0;
      }
    }

    var vendorConsents = getVendorConsents(gdprConsent.vendorData);

    if (vendorConsents == null || vendorConsents[CONSTANTS.INVIBES_VENDOR_ID.toString(10)] == null) {
      return 4;
    }

    if (vendorConsents[CONSTANTS.INVIBES_VENDOR_ID.toString(10)] === false) {
      return 0;
    }
