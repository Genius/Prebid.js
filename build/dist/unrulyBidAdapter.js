...
var buildPrebidResponseAndInstallRenderer = function buildPrebidResponseAndInstallRenderer(bids) {
  return bids.filter(function (serverBid) {
    var hasConfig = !!__WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](serverBid, 'ext.renderer.config');
    var hasSiteId = !!__WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](serverBid, 'ext.renderer.config.siteId');
    if (!hasConfig) __WEBPACK_IMPORTED_MODULE_0__src_utils__["logError"](new Error('UnrulyBidAdapter: Missing renderer config.'));
    if (!hasSiteId) __WEBPACK_IMPORTED_MODULE_0__src_utils__["logError"](new Error('UnrulyBidAdapter: Missing renderer siteId.'));
    return hasSiteId;
