...
function _mapAdUnitPathToElementId(adUnitCode) {
  if (__WEBPACK_IMPORTED_MODULE_2__src_utils_js__["isGptPubadsDefined"]()) {
    // eslint-disable-next-line no-undef
    var adSlots = googletag.pubads().getSlots();
    var isMatchingAdSlot = __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["isSlotMatchingAdUnitCode"](adUnitCode);

    for (var i = 0; i < adSlots.length; i++) {
      if (isMatchingAdSlot(adSlots[i])) {
        var id = adSlots[i].getSlotElementId();
        __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["logInfo"]("[33Across Adapter] Map ad unit path to HTML element id: '".concat(adUnitCode, "' -> ").concat(id));
        return id;
      }
    }
  }

  __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["logWarn"]("[33Across Adapter] Unable to locate element for ad unit code: '".concat(adUnitCode, "'"));
  return null;
}
...
function _createServerRequest(_ref2) {
  var bidRequest = _ref2.bidRequest,
      _ref2$gdprConsent = _ref2.gdprConsent,
      gdprConsent = _ref2$gdprConsent === void 0 ? {} : _ref2$gdprConsent,
      uspConsent = _ref2.uspConsent,
  ...
  var format; // We support size based bidfloors so obtain one if there's a rule associated

  if (typeof bidRequest.getFloor === 'function') {
    var getFloor = bidRequest.getFloor.bind(bidRequest);
    format = sizes.map(function (size) {
      var formatExt = _getBidFloors(getFloor, size);

      return _extends({}, size, formatExt);
    });
  } else {
    format = sizes;
  }
  ...
  var viewabilityAmount = _isViewabilityMeasurable(element) ? _getViewability(element, __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["getWindowTop"](), minSize) : NON_MEASURABLE;
  ...
  var ttxSettings = __WEBPACK_IMPORTED_MODULE_1__src_config_js__["b" /* config */].getConfig('ttxSettings');
...
function _createSync(_ref3) {
  var _ref3$siteId = _ref3.siteId,
      siteId = _ref3$siteId === void 0 ? 'zzz000000000003zzz' : _ref3$siteId,
      _ref3$gdprConsent = _ref3.gdprConsent,
      gdprConsent = _ref3$gdprConsent === void 0 ? {} : _ref3$gdprConsent,
  ...
  var sync = {
    type: 'iframe',
    url: "".concat(syncUrl, "&id=").concat(siteId, "&gdpr_consent=").concat(encodeURIComponent(consentString), "&us_privacy=").concat(encodeURIComponent(uspConsent))
  };
...
function _transformSizes(sizes) {
  if (__WEBPACK_IMPORTED_MODULE_2__src_utils_js__["isArray"](sizes) && sizes.length === 2 && !__WEBPACK_IMPORTED_MODULE_2__src_utils_js__["isArray"](sizes[0])) {
...
function _isIframe() {
  try {
    return __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["getWindowSelf"]() !== __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["getWindowTop"]();
  } catch (e) {
    return true;
  }
}
...
function buildRequests(bidRequests, bidderRequest) {
  ...
  var pageUrl = bidderRequest && bidderRequest.refererInfo ? bidderRequest.refererInfo.referer : undefined;
...
function getUserSyncs(syncOptions, responses, gdprConsent, uspConsent) {
  var syncUrls = syncOptions.iframeEnabled ? adapterState.uniqueSiteIds.map(function (siteId) {
    return _createSync({
      gdprConsent: gdprConsent,
      uspConsent: uspConsent,
      siteId: siteId
