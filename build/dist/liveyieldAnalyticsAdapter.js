...
var liveyield = _extends(Object(__WEBPACK_IMPORTED_MODULE_0__src_AnalyticsAdapter_js__["a" /* default */])({
    ...
    switch (eventType) {
      ...
      case BID_WON:
        ...
        try {
          ...
          if (!ad) {
            __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('Cannot find ad by unit name: ' + liveyield.instanceConfig.getAdUnitName(liveyield.instanceConfig.getPlacementOrAdUnitCode(args, prebidVersion)));
            break;
          }

          if (!args.bidderCode || !args.cpm) {
            __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('Bidder code or cpm is not valid');
            break;
          }
...
liveyield.enableAnalytics = function (config) {
  if (!config || !config.provider || config.provider !== 'liveyield') {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('expected config.provider to equal liveyield');
    return;
  }

  if (!config.options) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('options must be defined');
    return;
  }

  if (!config.options.customerId) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('options.customerId is required');
    return;
  }

  if (!config.options.customerName) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('options.customerName is required');
    return;
  }

  if (!config.options.customerSite) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('options.customerSite is required');
    return;
  }

  if (!config.options.sessionTimezoneOffset) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]('options.sessionTimezoneOffset is required');
    return;
  }
  ...
  if (typeof window[liveyield.instanceConfig.rtaFunctionName] !== 'function') {
    __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logError"]("Function ".concat(liveyield.instanceConfig.rtaFunctionName, " is not defined.") + "Make sure that LiveYield snippet in included before the Prebid Analytics configuration.");
    return;
  }
