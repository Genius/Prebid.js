...
var url = 'https://adxpremium.services/graphql'; // Events needed
...
var _CONSTANTS$EVENTS = __WEBPACK_IMPORTED_MODULE_3__src_constants_json___default.a.EVENTS,
    ...
    AUCTION_END = _CONSTANTS$EVENTS.AUCTION_END; // Memory objects
...
var adxpremiumAnalyticsAdapter = _extends(Object(__WEBPACK_IMPORTED_MODULE_1__src_AnalyticsAdapter__["a" /* default */])({
  url: url,
...
function auctionInit(args) {
  ...
  try {
    completeObject.referer = args.bidderRequests[0].refererInfo.referer.split('?')[0];
  } catch (e) {
...
function sendEvent(completeObject) {
  try {
    ...
    Object(__WEBPACK_IMPORTED_MODULE_0__src_ajax__["a" /* ajax */])(url, function () {
      console.log(Date.now() + ' Sending event to adxpremium server.');
    }, dataToSend, {
...
adxpremiumAnalyticsAdapter.enableAnalytics = function (config) {
  ...
  if (!config.options.pubId) {
    __WEBPACK_IMPORTED_MODULE_4__src_utils__["logError"]('Publisher ID (pubId) option is not defined. Analytics won\'t work');
    return;
  }
