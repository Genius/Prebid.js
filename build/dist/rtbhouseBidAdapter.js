...
var spec = {
  code: BIDDER_CODE,
  supportedMediaTypes: [__WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["b" /* BANNER */], __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["c" /* NATIVE */]],
  isBidRequestValid: function isBidRequestValid(bid) {
    return !!(__WEBPACK_IMPORTED_MODULE_3_core_js_pure_features_array_includes_js___default()(REGIONS, bid.params.region) && bid.params.publisherId);
  },
  ...
  interpretResponse: function interpretResponse(serverResponse, originalRequest) {
    ...
    if (!__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isArray"](responseBody)) {
      return [];
    }
...
function mapBanner(slot) {
  if (slot.mediaType === 'banner' || __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["deepAccess"](slot, 'mediaTypes.banner') || !slot.mediaType && !slot.mediaTypes) {
...
function mapSite(slot, bidderRequest) {
  ...
  return {
    ...
    name: __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getOrigin"]()
  };
}
/**
 * @param {object} slot Ad Unit Params by Prebid
...
function mapSource(slot) {
  ...
  var schain = mapSchain(slot.schain);
...
function mapSchain(schain) {
  ...
  if (!validateSchain(schain)) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logError"]('RTB House: required schain params missing');
    return null;
  }
...
function validateSchain(schain) {
  ...
  return schain.nodes.every(function (node) {
...
function mapNative(slot) {
  if (slot.mediaType === 'native' || __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["deepAccess"](slot, 'mediaTypes.native')) {
    return {
      request: {
...
function mapNativeAssets(slot) {
  var params = slot.nativeParams || __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["deepAccess"](slot, 'mediaTypes.native');
  var assets = [];
