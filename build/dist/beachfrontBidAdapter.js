...
var spec = {
  ...
  interpretResponse: function interpretResponse(response, _ref) {
    ...
    if (isVideoBid(bidRequest)) {
      if (!response || !response.url || !response.bidPrice) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"]("No valid video bids from ".concat(spec.code, " bidder"));
        return [];
      }
      ...
      return {
        ...
        mediaType: __WEBPACK_IMPORTED_MODULE_4__src_mediaTypes_js__["d" /* VIDEO */],
        currency: 'USD',
        netRevenue: true,
        ttl: 300
      };
      ...
      if (!response || !response.length) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"]("No valid banner bids from ".concat(spec.code, " bidder"));
        return [];
      }
      ...
      return response.filter(function (bid) {
        ...
        var request = __WEBPACK_IMPORTED_MODULE_5_core_js_pure_features_array_find_js___default()(bidRequest, function (req) {
        ...
        return {
          ...
          width: bid.w,
          height: bid.h,
          mediaType: __WEBPACK_IMPORTED_MODULE_4__src_mediaTypes_js__["b" /* BANNER */],
          currency: 'USD',
          netRevenue: true,
          ttl: 300
        };
  ...
  getUserSyncs: function getUserSyncs(syncOptions) {
    ...
    var uspConsent = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : '';
    var syncs = [];
    var gdprApplies = gdprConsent.gdprApplies,
        _gdprConsent$consentS = gdprConsent.consentString,
        consentString = _gdprConsent$consentS === void 0 ? '' : _gdprConsent$consentS;
    var bannerResponse = __WEBPACK_IMPORTED_MODULE_5_core_js_pure_features_array_find_js___default()(serverResponses, function (res) {
      return __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isArray"](res.body);
    });
    ...
    if (bannerResponse) {
      ...
      syncs.push({
        type: 'iframe',
        url: "https://sync.bfmio.com/sync_iframe?ifg=1&id=".concat(appId, "&gdpr=").concat(gdprApplies ? 1 : 0, "&gc=").concat(consentString, "&gce=1&us_privacy=").concat(uspConsent)
      });
    } else if (syncOptions.pixelEnabled) {
      syncs.push({
        type: 'image',
        url: "https://sync.bfmio.com/syncb?pid=144&id=".concat(appId, "&gdpr=").concat(gdprApplies ? 1 : 0, "&gc=").concat(consentString, "&gce=1&us_privacy=").concat(uspConsent)
      });
    }
...
function getOsVersion() {
  ...
  var cs = __WEBPACK_IMPORTED_MODULE_5_core_js_pure_features_array_find_js___default()(clientStrings, function (cs) {
...
function getPlayerBidParam(bid, key, defaultValue) {
  var param = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["deepAccess"](bid, 'params.player.' + key);
...
function getVideoTargetingParams(bid) {
  return Object.keys(Object(bid.params.video)).filter(function (param) {
    return __WEBPACK_IMPORTED_MODULE_6_core_js_pure_features_array_includes_js___default()(VIDEO_TARGETING, param);
...
function createVideoRequestData(bid, bidderRequest) {
  ...
  var tagid = getVideoBidParam(bid, 'tagid');
