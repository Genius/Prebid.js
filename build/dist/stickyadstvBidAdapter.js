...
var StickyAdsTVAdapter = function StickyAdsTVAdapter() {
  ...
  var formatInBannerHTML = function formatInBannerHTML(bid, size) {
    ...
    'var topWindow = (function(){var res=window; try{while(top != res){if(res.parent.location.href.length)res=res.parent;}}catch(e){}return res;})();' + 'var vast =  topWindow.stickyadstv_cache["' + placementCode + '"];' + 'var config = {' + '  preloadedVast:vast,' + '  autoPlay:true' + '};' + 'var ad = new topWindow.com.stickyadstv.vpaid.Ad(document.getElementById("stickyadstv_prebid_target"),config);' + 'if(topWindow.stickyadstv_asLoader) topWindow.stickyadstv_asLoader.registerEvents(ad);' + 'ad.initAd(' + size[0] + ',' + size[1] + ',"",0,"","");' + '</script>';

    return divHtml + script;
  };

  var formatOutstreamHTML = function formatOutstreamHTML(bid) {
    var placementCode = bid.placementCode;
    ...
    var script = "<script type='text/javascript'>" +
    // get the top most accessible window
    'var topWindow = (function(){var res=window; try{while(top != res){if(res.parent.location.href.length)res=res.parent;}}catch(e){}return res;})();' + 'var vast =  topWindow.stickyadstv_cache["' + placementCode + '"];' + 'var config = {' + '  preloadedVast:vast,' + '  ASLoader:topWindow.stickyadstv_asLoader';
    ...
    script += '};' + 'topWindow.com.stickyadstv.' + getAPIName(bid.params.format) + '.start(config);' + '</script>';
