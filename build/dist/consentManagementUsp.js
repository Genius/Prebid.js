...
function requestBidsHook(fn, reqBidsConfigObj) {
  ...
  if (!uspCallMap[consentAPI]) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"]("USP framework (".concat(consentAPI, ") is not a supported framework. Aborting consentManagement module and resuming auction."));
...
function storeUspConsentData(consentObject) {
  if (consentObject && consentObject.usPrivacy) {
    ...
    __WEBPACK_IMPORTED_MODULE_2__src_adapterManager__["uspDataHandler"].setConsentData(consentData);
  }
}
/**
...
function resetConsentData() {
  ...
  __WEBPACK_IMPORTED_MODULE_2__src_adapterManager__["uspDataHandler"].setConsentData(null);
}
/**
...
function setConsentConfig(config) {
  ...
  if (!config || _typeof(config) !== 'object') {
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"]('consentManagement.usp config not defined, exiting usp consent manager');
    return;
  }
  ...
  if (__WEBPACK_IMPORTED_MODULE_0__src_utils__["isStr"](config.cmpApi)) {
    ...
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logInfo"]("consentManagement.usp config did not specify cmpApi. Using system default setting (".concat(DEFAULT_CONSENT_API, ")."));
  }
  ...
  if (__WEBPACK_IMPORTED_MODULE_0__src_utils__["isNumber"](config.timeout)) {
    ...
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logInfo"]("consentManagement.usp config did not specify timeout. Using system default setting (".concat(DEFAULT_CONSENT_TIMEOUT, ")."));
  }

  __WEBPACK_IMPORTED_MODULE_0__src_utils__["logInfo"]('USPAPI consentManagement module has been activated...');
