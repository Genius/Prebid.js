...
var spec = {
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    ...
    for (var i = 0; i < validBidRequests.length; i++) {
      ...
      var queryString = '';
      var slotKey = __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["getBidIdParameter"]('slotKey', bid.params);
      queryString = __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["tryAppendQueryString"](queryString, 'slot_key', slotKey);
      queryString = __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["tryAppendQueryString"](queryString, 'imp_id', generateImpId());
      ...
      if (bannerAd.imps) {
        try {
          bannerAd.imps.forEach(function (impTrackUrl) {
            var tracker = __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["createTrackPixelHtml"](impTrackUrl);
            bid.ad += tracker;
          });
        } catch (error) {
          __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["logError"]('Error appending imp tracking pixel', error);
        }
      }
    }
  ...
  onBidWon: function onBidWon(bid) {
    ...
    Object(__WEBPACK_IMPORTED_MODULE_1__src_ajax_js__["a" /* ajax */])(winUrl, null);
  }
};
...
function newRenderer(bidderResponse) {
  ...
  try {
    ...
    __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["logWarn"]('Prebid Error calling setRender on newRenderer', err);
  }
