...
var spec = {
  ...
  interpretResponse: function interpretResponse(serverResponse, request) {
    ...
    if (response && response.seatbid && response.seatbid.length === 1 && response.seatbid[0].bid && response.seatbid[0].bid.length) {
      ...
      __WEBPACK_IMPORTED_MODULE_1__src_utils__["logInfo"]('padsquad.interpretResponse :: no valid responses to interpret');
    }

    return bidResponses;
  },
  getUserSyncs: function getUserSyncs(syncOptions, serverResponses) {
    __WEBPACK_IMPORTED_MODULE_1__src_utils__["logInfo"]('padsquad.getUserSyncs', 'syncOptions', syncOptions, 'serverResponses', serverResponses);
    var syncs = [];
