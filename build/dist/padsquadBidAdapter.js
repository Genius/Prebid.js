...
var ENDPOINT_URL = 'https://x.padsquad.com/auction';
...
var spec = {
  ...
  interpretResponse: function interpretResponse(serverResponse, request) {
    var bidResponses = [];
    var response = (serverResponse || {}).body; // response is always one seat with (optional) bids for each impression
    ...
    if (response && response.seatbid && response.seatbid.length === 1 && response.seatbid[0].bid && response.seatbid[0].bid.length) {
      ...
      __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"]('padsquad.interpretResponse :: no valid responses to interpret');
    }

    return bidResponses;
  },
  getUserSyncs: function getUserSyncs(syncOptions, serverResponses) {
    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"]('padsquad.getUserSyncs', 'syncOptions', syncOptions, 'serverResponses', serverResponses);
    var syncs = [];
