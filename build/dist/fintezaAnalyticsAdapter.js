...
function getPageInfo() {
  ...
  if (document.referrer) {
    pageInfo.referrerDomain = Object(__WEBPACK_IMPORTED_MODULE_4__src_url__["c" /* parse */])(document.referrer).hostname;
  }
...
function getTrackRequestLastTime() {
  ...
  try {
    if (window.localStorage) {
      return parseInt(window.localStorage.getItem(TRACK_TIME_KEY) || 0, 10);
    }
...
function sendTrackRequest(trackData) {
  try {
    Object(__WEBPACK_IMPORTED_MODULE_0__src_ajax__["a" /* ajax */])(fntzAnalyticsAdapter.context.host, null, trackData, {
      method: 'GET',
    ...
    __WEBPACK_IMPORTED_MODULE_3__src_utils__["logError"]('Error on send data: ', err);
  }
}

var fntzAnalyticsAdapter = _extends(Object(__WEBPACK_IMPORTED_MODULE_1__src_AnalyticsAdapter__["a" /* default */])({
...
fntzAnalyticsAdapter.enableAnalytics = function (config) {
  if (!config.options.id) {
    __WEBPACK_IMPORTED_MODULE_3__src_utils__["logError"]('Client ID (id) option is not defined. Analytics won\'t work');
    return;
  }
