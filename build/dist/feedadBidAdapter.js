...
var MEDIA_TYPES = [__WEBPACK_IMPORTED_MODULE_2__src_mediaTypes__["d" /* VIDEO */], __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes__["b" /* BANNER */]];
/**
 * Tag for logging
 * @type {string}
 */
...
function isBidRequestValid(bid) {
  var clientToken = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'params.clientToken');

  if (!clientToken || !isValidClientToken(clientToken)) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"](TAG, "missing or invalid parameter 'clientToken'. found value:", clientToken);
    return false;
  }

  var placementId = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'params.placementId');

  if (!placementId || !isValidPlacementId(placementId)) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logWarn"](TAG, "missing or invalid parameter 'placementId'. found value:", placementId);
    return false;
  }
...
function trackingHandlerFactory(klass) {
  return function (data) {
    ...
    if (params) {
      Object(__WEBPACK_IMPORTED_MODULE_3__src_ajax__["a" /* ajax */])("".concat(API_ENDPOINT).concat(API_PATH_TRACK_REQUEST), null, JSON.stringify(params), {
