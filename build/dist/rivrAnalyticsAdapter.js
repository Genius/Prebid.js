...
var rivrAnalytics = _extends(Object(__WEBPACK_IMPORTED_MODULE_1__src_AnalyticsAdapter_js__["a" /* default */])({
  ...
  track: function track(_ref) {
    ...
    if (window.rivraddon && window.rivraddon.analytics && window.rivraddon.analytics.getContext() && window.rivraddon.analytics.trackPbjsEvent) {
      __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["logInfo"]("ARGUMENTS FOR TYPE: ============= ".concat(eventType), args);
      window.rivraddon.analytics.trackPbjsEvent({
        eventType: eventType,
        args: args
      });
    }
...
rivrAnalytics.enableAnalytics = function (config) {
  if (window.rivraddon && window.rivraddon.analytics) {
    window.rivraddon.analytics.enableAnalytics(config, {
      utils: __WEBPACK_IMPORTED_MODULE_3__src_utils_js__,
      ajax: __WEBPACK_IMPORTED_MODULE_0__src_ajax_js__["a" /* ajax */],
      pbjsGlobalVariable: pbjs
    });
