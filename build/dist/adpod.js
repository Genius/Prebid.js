...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__src_auction__ = __webpack_require__(41);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_2__src_prebid__ = __webpack_require__(82);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__src_video__ = __webpack_require__(32);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_4__src_hook__ = __webpack_require__(13);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__src_videoCache__ = __webpack_require__(81);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8_core_js_library_fn_set__ = __webpack_require__(63);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_10__src_auctionManager__ = __webpack_require__(23);
...
function getPricePartForAdpodKey(bid) {
  var pricePart;
  var prioritizeDeals = __WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig('adpod.prioritizeDeals');

  if (prioritizeDeals && __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'video.dealTier')) {
    var adpodDealPrefix = __WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig("adpod.dealTier.".concat(bid.bidderCode, ".prefix"));
    pricePart = adpodDealPrefix ? adpodDealPrefix + __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'video.dealTier') : __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'video.dealTier');
  } else {
    var granularity = Object(__WEBPACK_IMPORTED_MODULE_1__src_auction__["h" /* getPriceGranularity */])(bid.mediaType);
    pricePart = Object(__WEBPACK_IMPORTED_MODULE_1__src_auction__["g" /* getPriceByGranularity */])(granularity)(bid);
  }
...
function callPrebidCacheHook(fn, auctionInstance, bidResponse, afterBidAdded, bidderRequest) {
  ...
  if (videoConfig && videoConfig.context === __WEBPACK_IMPORTED_MODULE_7__src_mediaTypes__["a" /* ADPOD */]) {
    var brandCategoryExclusion = __WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig('adpod.brandCategoryExclusion');
    ...
    if (!adServerCatId && brandCategoryExclusion) {
      ...
      if (__WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig('adpod.deferCaching') === false) {
...
function checkVideoBidSetupHook(fn, bid, bidRequest, videoMediaType, context) {
  if (context === __WEBPACK_IMPORTED_MODULE_7__src_mediaTypes__["a" /* ADPOD */]) {
    ...
    if (!__WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig('cache.url') && bid.vastXml && !bid.vastUrl) {
...
__WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig('adpod', function (config) {
...
function initAdpodHooks() {
  Object(__WEBPACK_IMPORTED_MODULE_4__src_hook__["d" /* setupBeforeHookFnOnce */])(__WEBPACK_IMPORTED_MODULE_1__src_auction__["e" /* callPrebidCache */], callPrebidCacheHook);
  Object(__WEBPACK_IMPORTED_MODULE_4__src_hook__["d" /* setupBeforeHookFnOnce */])(__WEBPACK_IMPORTED_MODULE_2__src_prebid__["checkAdUnitSetup"], checkAdUnitSetupHook);
  Object(__WEBPACK_IMPORTED_MODULE_4__src_hook__["d" /* setupBeforeHookFnOnce */])(__WEBPACK_IMPORTED_MODULE_3__src_video__["c" /* checkVideoBidSetup */], checkVideoBidSetupHook);
}
...
function getTargeting() {
  ...
  if (!callback) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logError"]('No callback function was defined in the getTargeting call.  Aborting getTargeting().');
    return;
  }

  codes = codes || [];
  var adPodAdUnits = getAdPodAdUnits(codes);
  var bidsReceived = __WEBPACK_IMPORTED_MODULE_10__src_auctionManager__["a" /* auctionManager */].getBidsReceived();
  var competiveExclusionEnabled = __WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig('adpod.brandCategoryExclusion');
  var deferCachingSetting = __WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig('adpod.deferCaching');
  ...
  if (prioritizeDeals) {
    var _bids$reduce = bids.reduce(function (partitions, bid) {
      var bidDealTier = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](bid, 'video.dealTier');
      var minDealTier = __WEBPACK_IMPORTED_MODULE_6__src_config__["b" /* config */].getConfig("adpod.dealTier.".concat(bid.bidderCode, ".minDealTier"));
  ...
  if (deferCachingEnabled === false) {
    adPodAdUnits.forEach(function (adUnit) {
      var adPodTargeting = [];
      var adPodDurationSeconds = __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](adUnit, 'mediaTypes.video.adPodDurationSec');
    ...
    callPrebidCacheAfterAuction(bidsToCache, function (error, bidsSuccessfullyCached) {
      if (error) {
        ...
        var groupedBids = __WEBPACK_IMPORTED_MODULE_0__src_utils__["groupBy"](bidsSuccessfullyCached, 'adUnitCode');
...
function getAdPodAdUnits(codes) {
  return __WEBPACK_IMPORTED_MODULE_10__src_auctionManager__["a" /* auctionManager */].getAdUnits().filter(function (adUnit) {
    return __WEBPACK_IMPORTED_MODULE_0__src_utils__["deepAccess"](adUnit, 'mediaTypes.video.context') === __WEBPACK_IMPORTED_MODULE_7__src_mediaTypes__["a" /* ADPOD */];
...
function getExclusiveBids(bidsReceived) {
  ...
  bids = __WEBPACK_IMPORTED_MODULE_0__src_utils__["groupBy"](bids, TARGETING_KEY_PB_CAT_DUR);
  var filteredBids = [];
  Object.keys(bids).forEach(function (targetingKey) {
    bids[targetingKey].sort(__WEBPACK_IMPORTED_MODULE_0__src_utils__["compareOn"]('responseTimestamp'));
...
function getBidsForAdpod(bidsReceived, adPodAdUnits) {
  ...
  return bidsReceived.filter(function (bid) {
    return adUnitCodes.indexOf(bid.adUnitCode) != -1 && bid.video && bid.video.context === __WEBPACK_IMPORTED_MODULE_7__src_mediaTypes__["a" /* ADPOD */];
  });
}
...
Object(__WEBPACK_IMPORTED_MODULE_4__src_hook__["c" /* module */])('adpod', function shareAdpodUtilities() {
  if (!__WEBPACK_IMPORTED_MODULE_0__src_utils__["isPlainObject"](arguments.length <= 0 ? undefined : arguments[0])) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["logError"]('Adpod module needs plain object to share methods with submodule');
    return;
  }
