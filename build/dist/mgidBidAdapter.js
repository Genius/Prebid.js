...
__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["_each"](NATIVE_ASSETS, function (anAsset) {
...
var spec = {
  ...
  isBidRequestValid: function isBidRequestValid(bid) {
    var banner = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'mediaTypes.banner');
    var native = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'mediaTypes.native');
    var nativeOk = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isPlainObject"](native);

    if (nativeOk) {
      var nativeParams = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'nativeParams');
      var assetsCount = 0;

      if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isPlainObject"](nativeParams)) {
        for (var k in nativeParams) {
          ...
          if (!__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isPlainObject"](v) || !supportProp && __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](v, 'required')) {
            nativeOk = false;
            break;
          }
        }
    ...
    var bannerOk = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isPlainObject"](banner);

    if (bannerOk) {
      var sizes = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](banner, 'sizes');
      bannerOk = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](sizes) && sizes.length > 0;
    ...
    return (bannerOk || nativeOk) && __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isPlainObject"](bid.params) && !!bid.adUnitCode && __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](bid.adUnitCode) && (plcmt > 0 ? bid.params.placementId.toString().search(spec.reId) === 0 : true) && !!acc && acc > 0 && bid.params.accountId.toString().search(spec.reId) === 0;
  },
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"](LOG_INFO_PREFIX + "buildRequests");
    ...
    var info = pageInfo();
    var page = info.location || __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bidderRequest, 'refererInfo.referer') || __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bidderRequest, 'refererInfo.canonicalUrl');
    var hostname = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["parseUrl"](page).hostname;
    ...
    if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](muid) && muid.length > 0) {
      url += '?muid=' + muid;
    }

    var cur = [setOnAny(validBidRequests, 'params.currency') || setOnAny(validBidRequests, 'params.cur') || __WEBPACK_IMPORTED_MODULE_3__src_config_js__["b" /* config */].getConfig('currency.adServerCurrency') || DEFAULT_CUR];
    var secure = window.location.protocol === 'https:' ? 1 : 0;
    var imp = [];
    validBidRequests.forEach(function (bid) {
      var tagid = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'params.placementId') || 0;
      ...
      var bidFloor = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'params.bidFloor') || __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'params.bidfloor') || 0;

      if (bidFloor && __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isNumber"](bidFloor)) {
    ...
    var request = {
      id: __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bidderRequest, 'bidderRequestId'),
      site: {
        domain: domain,
        page: page
      },
    ...
    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"](LOG_INFO_PREFIX + "buildRequest:", request);
    return {
      method: 'POST',
      url: url,
  ...
  interpretResponse: function interpretResponse(serverResponse, bidRequests) {
    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"](LOG_INFO_PREFIX + "interpretResponse", serverResponse);

    if (serverResponse == null || serverResponse.body == null || serverResponse.body === '' || !__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](serverResponse.body.seatbid) || !serverResponse.body.seatbid.length) {
      return;
    }

    var returnedBids = [];
    var muidn = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](serverResponse.body, 'ext.muidn');

    if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](muidn) && muidn.length > 0) {
    ...
    serverResponse.body.seatbid.forEach(function (bids) {
      bids.bid.forEach(function (bid) {
        ...
        if (pbid.mediaType === __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["c" /* NATIVE */] && __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isEmpty"](pbid.native)) {
          return;
        }
    ...
    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"](LOG_INFO_PREFIX + "interpretedResponse", returnedBids);
    return returnedBids;
  },
  onBidWon: function onBidWon(bid) {
    var cpm = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'adserverTargeting.hb_pb') || '';

    if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](bid.nurl) && bid.nurl !== '') {
      bid.nurl = bid.nurl.replace(/\${AUCTION_PRICE}/, cpm);
      __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["triggerPixel"](bid.nurl);
    }

    if (bid.isBurl) {
      if (bid.mediaType === __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["b" /* BANNER */]) {
        bid.ad = bid.ad.replace(/\${AUCTION_PRICE}/, cpm);
      } else {
        bid.burl = bid.burl.replace(/\${AUCTION_PRICE}/, cpm);
        __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["triggerPixel"](bid.burl);
      }
    }

    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"](LOG_INFO_PREFIX + "onBidWon");
  },
  getUserSyncs: function getUserSyncs(syncOptions, serverResponses) {
    __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logInfo"](LOG_INFO_PREFIX + "getUserSyncs");
  }
};
...
function setOnAny(collection, key) {
  for (var i = 0, result; i < collection.length; i++) {
    result = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](collection[i], key);
...
function prebidBid(serverBid, cur) {
  if (!__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](cur) || cur === '') {
    cur = DEFAULT_CUR;
  }
  ...
  var bid = {
    ...
    nurl: serverBid.nurl || '',
    burl: serverBid.burl || '',
    isBurl: __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isStr"](serverBid.burl) && serverBid.burl.length > 0
  };
...
function setMediaType(bid, newBid) {
  if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'ext.crtype') === 'native') {
...
function createBannerRequest(bid) {
  var sizes = __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](bid, 'mediaTypes.banner.sizes');
  var format = [];
      ...
      if (!(nativeRequestObject.assets && nativeRequestObject.assets.length > 0 && nativeRequestObject.assets.hasOwnProperty(key))) {
        switch (key) {
          ...
          case NATIVE_ASSETS.IMAGE.KEY:
            var wmin = params[key].wmin || params[key].minimumWidth || (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](params[key].minsizes) && params[key].minsizes.length > 0 ? params[key].minsizes[0] : 0);
            var hmin = params[key].hmin || params[key].minimumHeight || (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](params[key].minsizes) && params[key].minsizes.length > 1 ? params[key].minsizes[1] : 0);
            assetObj = {
              id: NATIVE_ASSETS.IMAGE.ID,
              required: params[key].required ? 1 : 0,
              img: {
                type: NATIVE_ASSET_IMAGE_TYPE.IMAGE,
                w: params[key].w || params[key].width || (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](params[key].sizes) && params[key].sizes.length > 0 ? params[key].sizes[0] : 0),
                h: params[key].h || params[key].height || (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](params[key].sizes) && params[key].sizes.length > 1 ? params[key].sizes[1] : 0),
          ...
          case NATIVE_ASSETS.ICON.KEY:
            assetObj = {
              ...
              img: {
                ...
                w: params[key].w || params[key].width || (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](params[key].sizes) && params[key].sizes.length > 0 ? params[key].sizes[0] : 0),
                h: params[key].h || params[key].height || (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isArray"](params[key].sizes) && params[key].sizes.length > 0 ? params[key].sizes[1] : 0)
              }
            };
    ...
    for (var i = 0; i < lengthOfExistingAssets; i++) {
      if (ele.id === nativeRequestObject.assets[i].id) {
        ...
        if (ele.id === 4 && nativeRequestObject.assets[i].id === 11) {
          if (__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"](nativeRequestObject.assets[i], 'data.type') === ele.data.type) {
            presentrequiredAssetCount++;
            break;
          }
        }
...
function parseNativeResponse(bid, newBid) {
  ...
  if (bid.hasOwnProperty('adm')) {
    ...
    try {
      ...
      __WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logWarn"](LOG_WARN_PREFIX + 'Error: Cannot parse native response for ad response: ' + newBid.adm);
      return;
    }
