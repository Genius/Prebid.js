...
var spec = {
  ...
  buildRequests: function buildRequests(bidRequests, bidderRequest) {
    ...
    var userObjBid = __WEBPACK_IMPORTED_MODULE_3_core_js_pure_features_array_find_js___default()(bidRequests, hasUserInfo);
    var userObj;

    if (userObjBid) {
      userObj = {};
      Object.keys(userObjBid.params.user).filter(function (param) {
        return __WEBPACK_IMPORTED_MODULE_4_core_js_pure_features_array_includes_js___default()(USER_PARAMS, param);
      }).forEach(function (param) {
        return userObj[param] = userObjBid.params.user[param];
      });
    }

    var appDeviceObjBid = __WEBPACK_IMPORTED_MODULE_3_core_js_pure_features_array_find_js___default()(bidRequests, hasAppDeviceInfo);
    var appDeviceObj;

    if (appDeviceObjBid && appDeviceObjBid.params && appDeviceObjBid.params.app) {
      appDeviceObj = {};
      Object.keys(appDeviceObjBid.params.app).filter(function (param) {
        return __WEBPACK_IMPORTED_MODULE_4_core_js_pure_features_array_includes_js___default()(APP_DEVICE_PARAMS, param);
      }).forEach(function (param) {
        return appDeviceObj[param] = appDeviceObjBid.params.app[param];
      });
    }

    var appIdObjBid = __WEBPACK_IMPORTED_MODULE_3_core_js_pure_features_array_find_js___default()(bidRequests, hasAppId);
    var appIdObj;
  ...
  interpretResponse: function interpretResponse(serverResponse, _ref) {
    ...
    if (serverResponse.tags) {
      serverResponse.tags.forEach(function (serverBid) {
        ...
        if (rtbBid) {
          if (rtbBid.cpm !== 0 && __WEBPACK_IMPORTED_MODULE_4_core_js_pure_features_array_includes_js___default()(_this.supportedMediaTypes, rtbBid.ad_type)) {
...
function formatRequest(payload, bidderRequest) {
  ...
  if (payload.tags.length > MAX_IMPS_PER_REQUEST) {
    ...
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["chunk"](payload.tags, MAX_IMPS_PER_REQUEST).forEach(function (tags) {
...
function newBid(serverBid, rtbBid, bidderRequest) {
  ...
  if (rtbBid.rtb.video) {
    ...
    try {
      ...
      var tracker = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["createTrackPixelHtml"](url);
      bid.ad += tracker;
    } catch (error) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logError"]('Error appending tracking pixel', error);
    }
  }
...
function bidToTag(bid) {
  ...
  var videoMediaType = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["deepAccess"](bid, "mediaTypes.".concat(__WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */]));
  var context = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["deepAccess"](bid, 'mediaTypes.video.context');

  if (bid.mediaType === __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */] || videoMediaType) {
    tag.ad_types.push(__WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */]);
  } // instream gets vastUrl, outstream gets vastXml


  if (bid.mediaType === __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */] || videoMediaType && context !== 'outstream') {
  ...
  if (__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isEmpty"](bid.mediaType) && __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isEmpty"](bid.mediaTypes) || bid.mediaType === __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["b" /* BANNER */] || bid.mediaTypes && bid.mediaTypes[__WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["b" /* BANNER */]]) {
...
function getRtbBid(tag) {
  return tag && tag.ads && tag.ads.length && __WEBPACK_IMPORTED_MODULE_3_core_js_pure_features_array_find_js___default()(tag.ads, function (ad) {
    return ad.rtb;
  });
}
