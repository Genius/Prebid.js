...
var PubmaticAdapter = function PubmaticAdapter() {
  ...
  var usersync = false;
  var _secure = 0;
  var _protocol = (window.location.protocol === 'https:' ? (_secure = 1, 'https') : 'http') + '://';
  var iframe = void 0;
  ...
  function _initConf() {
    ...
    conf.kltstamp = currTime.getFullYear() + '-' + (currTime.getMonth() + 1) + '-' + currTime.getDate() + ' ' + currTime.getHours() + ':' + currTime.getMinutes() + ':' + currTime.getSeconds();
      ...
      if (customPars.hasOwnProperty(key)) {
        ...
        if (value) {
          ...
          if (utils.isStr(value)) {
            ...
            utils.logWarn('PubMatic: Ignoring param key: ' + customPars[key] + ', expects string-value, found ' + (typeof value === 'undefined' ? 'undefined' : _typeof(value)));
          }
        }
      }
    }
  ...
  function _legacyExecution(conf, slots) {
    ...
    content = content.replace('<!--POST_SCRIPT_TAG_MACRO-->', '<script>window.parent.pbjs.handlePubmaticCallback(window.bidDetailsMap, window.progKeyValueMap);</script>');
  ...
  pbjs.handlePubmaticCallback = function (bidDetailsMap, progKeyValueMap) {
    var i;
    var adUnit;
    var adUnitInfo;
    var bid;
