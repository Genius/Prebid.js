...
var USER_SYNC_URL = 'https://cdn.deepintent.com/syncpixel.html';
var DI_M_V = '1.0.0';
var spec = {
  code: BIDDER_CODE,
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    ...
    var openRtbBidRequest = {
      id: __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["generateUUID"](),
      at: 1,
    ...
    if (bidderRequest && bidderRequest.uspConsent) {
      __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["deepSetValue"](openRtbBidRequest, 'regs.ext.us_privacy', bidderRequest.uspConsent);
    }

    if (bidderRequest && bidderRequest.gdprConsent) {
      __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["deepSetValue"](openRtbBidRequest, 'user.ext.consent', bidderRequest.gdprConsent.consentString);
      __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["deepSetValue"](openRtbBidRequest, 'regs.ext.gdpr', bidderRequest.gdprConsent.gdprApplies ? 1 : 0);
    }
...
function buildBanner(bid) {
  if (__WEBPACK_IMPORTED_MODULE_2__src_utils_js__["deepAccess"](bid, 'mediaTypes.banner')) {
    // Get Sizes from MediaTypes Object, Will always take first size, will be overrided by params for exact w,h
    if (__WEBPACK_IMPORTED_MODULE_2__src_utils_js__["deepAccess"](bid, 'mediaTypes.banner.sizes') && !bid.params.height && !bid.params.width) {
      var sizes = __WEBPACK_IMPORTED_MODULE_2__src_utils_js__["deepAccess"](bid, 'mediaTypes.banner.sizes');

      if (__WEBPACK_IMPORTED_MODULE_2__src_utils_js__["isArray"](sizes) && sizes.length > 0) {
        return {
          h: sizes[0][1],
          w: sizes[0][0],
