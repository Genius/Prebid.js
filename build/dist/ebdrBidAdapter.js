...
var spec = {
  ...
  buildRequests: function buildRequests(bids) {
    ...
    bids.forEach(function (bid) {
      ...
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Log bid', bid);
      ...
      var _mediaTypes = bid.mediaTypes && bid.mediaTypes.video ? __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["d" /* VIDEO */] : __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["b" /* BANNER */];

      zoneid = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('zoneid', bid.params);
      ...
      ebdrParams['latitude'] = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('latitude', bid.params);
      ebdrParams['longitude'] = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('longitude', bid.params);
      ebdrParams['ifa'] = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('IDFA', bid.params).length > __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('ADID', bid.params).length ? __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('IDFA', bid.params) : __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('ADID', bid.params);
    });
    var ebdrBidReq = {
      id: requestId,
      imp: ebdrImps,
  ...
  interpretResponse: function interpretResponse(serverResponse, bidRequest) {
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Log serverResponse', serverResponse);
    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logInfo"]('Log bidRequest', bidRequest);
    ...
    ebdrResponseObj.seatbid[0].bid.forEach(function (ebdrBid) {
      ...
      if (bidRequest.bids[ebdrBid.id].mediaTypes == __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["b" /* BANNER */]) {
        adm = decodeURIComponent(ebdrBid.adm);
        type = 'ad';
        _mediaTypes = __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["b" /* BANNER */];
      } else {
        adm = ebdrBid.adm;
        type = 'vastXml';
        _mediaTypes = __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["d" /* VIDEO */];
...
function getWidthAndHeight(bid) {
  ...
  if (bid.mediaTypes && bid.mediaTypes[_mediaTypes]) {
    if (_mediaTypes == __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["b" /* BANNER */] && bid.mediaTypes[_mediaTypes].sizes && bid.mediaTypes[_mediaTypes].sizes[0] && bid.mediaTypes[_mediaTypes].sizes[0].length === 2) {
      adW = bid.mediaTypes[_mediaTypes].sizes[0][0];
      adH = bid.mediaTypes[_mediaTypes].sizes[0][1];
    } else if (_mediaTypes == __WEBPACK_IMPORTED_MODULE_1__src_mediaTypes_js__["d" /* VIDEO */] && bid.mediaTypes[_mediaTypes].playerSize && bid.mediaTypes[_mediaTypes].playerSize.length === 2) {
