  ...
  try {
    ...
    if (newUtm === false) {
      for (var _prop in utmKeys) {
        var itemValue = storage.getDataFromLocalStorage("pw-".concat(_prop));
      ...
      for (var _prop2 in utmKeys) {
        storage.setDataInLocalStorage("pw-".concat(_prop2), utmKeys[_prop2]);
      }
    }
  } catch (e) {
...
function sendEvent(eventType, data) {
  ...
  Object(__WEBPACK_IMPORTED_MODULE_0__src_ajax_js__["a" /* ajax */])(configOptions.endpoint, function (result) {
    return utils.logInfo("".concat(analyticsName, "Result"), result);
  }, JSON.stringify(dataBag));
}

var pubwiseAnalytics = _extends(Object(__WEBPACK_IMPORTED_MODULE_1__src_AnalyticsAdapter_js__["a" /* default */])({
