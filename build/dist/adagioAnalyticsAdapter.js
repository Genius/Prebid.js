...
var adagioEnqueue = function adagioEnqueue(action, data) {
  __WEBPACK_IMPORTED_MODULE_3__src_utils__["getWindowTop"]().ADAGIO.queue.push({
    action: action,
    data: data,
    ts: Date.now()
  });
...
function canAccessTopWindow() {
  try {
    if (__WEBPACK_IMPORTED_MODULE_3__src_utils__["getWindowTop"]().location.href) {
      return true;
    }
  } catch (error) {
    return false;
...
var adagioAdapter = _extends(Object(__WEBPACK_IMPORTED_MODULE_0__src_AnalyticsAdapter__["a" /* default */])({
  emptyUrl: emptyUrl,
...
adagioAdapter.enableAnalytics = function (config) {
  ...
  var w = __WEBPACK_IMPORTED_MODULE_3__src_utils__["getWindowTop"]();
  w.ADAGIO = w.ADAGIO || {};
