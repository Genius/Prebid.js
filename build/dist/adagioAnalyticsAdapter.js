...
var events = Object.keys(__WEBPACK_IMPORTED_MODULE_2__src_constants_json___default.a.EVENTS).map(function (key) {
  return __WEBPACK_IMPORTED_MODULE_2__src_constants_json___default.a.EVENTS[key];
});
var VERSION = '2.0.0';

var adagioEnqueue = function adagioEnqueue(action, data) {
  __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["getWindowTop"]().ADAGIO.queue.push({
    action: action,
    data: data,
    ts: Date.now()
  });
...
function canAccessTopWindow() {
  try {
    if (__WEBPACK_IMPORTED_MODULE_3__src_utils_js__["getWindowTop"]().location.href) {
      return true;
    }
  } catch (error) {
    return false;
...
var adagioAdapter = _extends(Object(__WEBPACK_IMPORTED_MODULE_0__src_AnalyticsAdapter_js__["a" /* default */])({
  emptyUrl: emptyUrl,
  ...
  track: function track(_ref) {
    ...
    if (typeof args !== 'undefined' && events.indexOf(eventType) !== -1) {
      adagioEnqueue('pb-analytics-event', {
        eventName: eventType,
        args: args
      });
    }
...
adagioAdapter.enableAnalytics = function (config) {
  if (!canAccessTopWindow()) {
    return;
  }

  var w = __WEBPACK_IMPORTED_MODULE_3__src_utils_js__["getWindowTop"]();
  w.ADAGIO = w.ADAGIO || {};
