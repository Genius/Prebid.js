...
var spec = {
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    ...
    __WEBPACK_IMPORTED_MODULE_0__src_utils__["_each"](validBidRequests, function (bidRequest) {
      ...
      if (sizes) {
        if (sizes && __WEBPACK_IMPORTED_MODULE_0__src_utils__["isArray"](sizes[0])) {
          req.imp[0].banner.w = sizes[0][0];
          req.imp[0].banner.h = sizes[0][1];
        } else if (sizes && __WEBPACK_IMPORTED_MODULE_0__src_utils__["isNumber"](sizes[0])) {
  ...
  interpretResponse: function interpretResponse(serverResponse, request) {
    ...
    if (serverResponse.body.seatbid && serverResponse.body.seatbid.length > 0 && serverResponse.body.seatbid[0].bid && serverResponse.body.seatbid[0].bid.length > 0 && serverResponse.body.seatbid[0].bid[0].adm) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils__["_each"](serverResponse.body.seatbid[0].bid, function (bodyAds) {
        var adstr = '';
        adstr = bodyAds.adm;
        var bidResponse = {
