...
var RENDERER_URL = 'https://js.brealtime.com/outstream/1.30.0/bundle.js';
var ADAPTER_VERSION = '1.5.1';
var DEFAULT_CUR = 'USD';
var emxAdapter = {
  validateSizes: function validateSizes(sizes) {
    if (!__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isArray"](sizes) || typeof sizes[0] === 'undefined') {
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': Sizes should be an array');
      return false;
    }

    return sizes.every(function (size) {
      return __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isArray"](size) && size.length === 2;
    });
  },
  ...
  buildBanner: function buildBanner(bid) {
    ...
    if (!emxAdapter.validateSizes(sizes)) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': could not detect mediaType banner sizes. Assigning to bid sizes instead');
      sizes = bid.sizes;
    }
  ...
  formatVideoResponse: function formatVideoResponse(bidResponse, emxBid, bidRequest) {
    ...
    if (bidRequest.bidderRequest && bidRequest.bidderRequest.bids && bidRequest.bidderRequest.bids.length > 0) {
      var matchingBid = __WEBPACK_IMPORTED_MODULE_5_core_js_pure_features_array_find_js___default()(bidRequest.bidderRequest.bids, function (bid) {
        return bidResponse.requestId && bid.bidId && bidResponse.requestId === bid.bidId && bid.mediaTypes && bid.mediaTypes.video && bid.mediaTypes.video.context === 'outstream';
      });
  ...
  cleanProtocols: function cleanProtocols(video) {
    if (video.protocols && __WEBPACK_IMPORTED_MODULE_4_core_js_pure_features_array_includes_js___default()(video.protocols, 7)) {
      // not supporting VAST protocol 7 (VAST 4.0);
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': VAST 4.0 is currently not supported. This protocol has been filtered out of the request.');
  ...
  parseResponse: function parseResponse(bidResponseAdm) {
    try {
      ...
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logError"]('emx_digitalBidAdapter', 'error', err);
    }
  },
  ...
  getSite: function getSite(refInfo) {
...
var spec = {
  ...
  isBidRequestValid: function isBidRequestValid(bid) {
    if (!bid || !bid.params) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': Missing bid or bid params.');
      return false;
    }

    if (bid.bidder !== BIDDER_CODE) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': Must use "emx_digital" as bidder code.');
      return false;
    }

    if (!bid.params.tagid || !__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["isStr"](bid.params.tagid)) {
      __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': Missing tagid param or tagid present and not type String.');
      return false;
    }
    ...
    if (bid.mediaTypes && bid.mediaTypes.banner) {
      ...
      if (!emxAdapter.validateSizes(sizes)) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': Missing sizes in bid');
        return false;
      }
    } else if (bid.mediaTypes && bid.mediaTypes.video) {
      if (!emxAdapter.checkVideoContext(bid)) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': Missing video context: instream or outstream');
        return false;
      }

      if (!bid.mediaTypes.video.playerSize) {
        __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["logWarn"](BIDDER_CODE + ': Missing video playerSize');
        return false;
      }
    }
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    ...
    var url = 'https://' + ENDPOINT + ('?t=' + timeout + '&ts=' + timestamp + '&src=pbjs');
    var secure = location.protocol.indexOf('https') > -1 ? 1 : 0;
    var device = emxAdapter.getDevice();
    var site = emxAdapter.getSite(bidderRequest.refererInfo);

    __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["_each"](validBidRequests, function (bid) {
      var tagid = __WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('tagid', bid.params);
      var bidfloor = parseFloat(__WEBPACK_IMPORTED_MODULE_0__src_utils_js__["getBidIdParameter"]('bidfloor', bid.params)) || 0;
    ...
    emxData = emxAdapter.getGdpr(bidderRequest, _extends({}, emxData));
  ...
  interpretResponse: function interpretResponse(serverResponse, bidRequest) {
    ...
    if (response.seatbid && response.seatbid.length > 0 && response.seatbid[0].bid) {
      response.seatbid.forEach(function (emxBid) {
        ...
        bidResponse.mediaType = isVideo ? __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["d" /* VIDEO */] : __WEBPACK_IMPORTED_MODULE_2__src_mediaTypes_js__["b" /* BANNER */];
