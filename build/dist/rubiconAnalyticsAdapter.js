...
var serverConfig;
__WEBPACK_IMPORTED_MODULE_4__src_config__["b" /* config */].getConfig('s2sConfig', function (_ref) {
...
function sendMessage(auctionId, bidWonId) {
  function formatBid(bid) {
    return __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bid, ['bidder', 'bidId', function (bidId) {
      return __WEBPACK_IMPORTED_MODULE_5__src_utils__["deepAccess"](bid, 'bidResponse.seatBidId') || bidId;
      ...
      return bidResponse ? __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bidResponse, ['bidPriceUSD', 'dealId', 'dimensions', 'mediaType']) : undefined;
    }]);
  }

  function formatBidWon(bid) {
    return _extends(formatBid(bid), __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bid.adUnit, ['adUnitCode', 'transactionId', 'videoAdFormat', function () {
  ...
  var referrer = __WEBPACK_IMPORTED_MODULE_4__src_config__["b" /* config */].getConfig('pageUrl') || __WEBPACK_IMPORTED_MODULE_5__src_utils__["getTopWindowUrl"]();
  var message = {
    eventTimeMillis: Date.now(),
    integration: __WEBPACK_IMPORTED_MODULE_4__src_config__["b" /* config */].getConfig('rubicon.int_type') || DEFAULT_INTEGRATION,
    version: "2.37.0",
    referrerUri: referrer
  };
  var wrapperName = __WEBPACK_IMPORTED_MODULE_4__src_config__["b" /* config */].getConfig('rubicon.wrapperName');
  ...
  if (auctionCache && !auctionCache.sent) {
    var adUnitMap = Object.keys(auctionCache.bids).reduce(function (adUnits, bidId) {
      ...
      if (!adUnit) {
        adUnit = adUnits[bid.adUnit.adUnitCode] = __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bid.adUnit, ['adUnitCode', 'transactionId', 'mediaTypes', 'dimensions', 'adserverTargeting', function () {
    ...
    Object.keys(auctionCache.bids).forEach(function (bidId) {
      ...
      _extends(auctionCache.bids[bidId], __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](adUnitMap[adCode], ['accountId', 'siteId', 'zoneId']));
    });
    var auction = {
...
function getBidPrice(bid) {
  ...
  __WEBPACK_IMPORTED_MODULE_5__src_utils__["logWarn"]('Rubicon Analytics Adapter: Could not determine the bidPriceUSD of the bid ', bid);
}

function parseBidResponse(bid, previousBidResponse) {
  ...
  return __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bid, ['bidPriceUSD', function () {
    return responsePrice;
  }, 'dealId', 'status', 'mediaType', 'dimensions', function () {
    return __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bid, ['width', 'height']);
  }, 'seatBidId']);
}
  ...
  track: function track(_ref2) {
    ...
    switch (eventType) {
      case AUCTION_INIT:
        ...
        var cacheEntry = __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](args, ['timestamp', 'timeout']);
        cacheEntry.bids = {};
        cacheEntry.bidsWon = {};
      ...
      case BID_REQUESTED:
        _extends(cache.auctions[args.auctionId].bids, args.bids.reduce(function (memo, bid) {
          ...
          memo[bid.bidId] = __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bid, ['bidder', function (bidder) {
          ...
          'finalSource as source', 'params', function (params, bid) {
            switch (bid.bidder) {
              ...
              case 'rubicon':
                return __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](params, ['accountId', 'siteId', 'zoneId']);
            }
            ...
            return __WEBPACK_IMPORTED_MODULE_5__src_utils__["pick"](bid, ['adUnitCode', 'transactionId', 'sizes as dimensions', function (sizes) {
      ...
      case BID_RESPONSE:
        ...
        bid.bidResponse = parseBidResponse(args, bid.bidResponse); // RP server banner overwrites bidId with bid.seatBidId

        if (__WEBPACK_IMPORTED_MODULE_5__src_utils__["deepAccess"](bid, 'bidResponse.seatBidId') && bid.bidder === 'rubicon' && bid.source === 'server' && ['video', 'banner'].some(function (i) {
          return __WEBPACK_IMPORTED_MODULE_5__src_utils__["deepAccess"](bid, 'bidResponse.mediaType') === i;
        })) {
