...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_5__src_userSync_js__ = __webpack_require__(43);
...
var PAGEVIEW_ID = Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["generateUUID"])();
...
var spec = {
  ...
  isBidRequestValid: function isBidRequestValid(bid) {
    ...
    if (!Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.banner') && !Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.video')) {
      return false;
    }

    if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.banner')) {
      // Sonobi does not support multi type bids, favor banner over video
      if (!Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.banner.sizes') && !bid.params.sizes) {
        // sizes at the banner or params level is required.
        return false;
      }
    } else if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.video')) {
      if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.video.context') === 'outstream' && !bid.params.sizes) {
        // bids.params.sizes is required for outstream video adUnits
        return false;
      }

      if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.video.context') === 'instream' && !Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.video.playerSize')) {
  ...
  buildRequests: function buildRequests(validBidRequests, bidderRequest) {
    var bids = validBidRequests.map(function (bid) {
      ...
      if (/^[\/]?[\d]+[[\/].+[\/]?]?$/.test(slotIdentifier)) {
        ...
        Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logError"])("The ad unit code or Sonobi Placement id for slot ".concat(bid.bidId, " is invalid"));
      }
    });
    var data = {};
    ...
    var payload = {
      ...
      's': Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["generateUUID"])(),
      'pv': PAGEVIEW_ID,
      'vp': _getPlatform(),
      'lib_name': 'prebid',
      'lib_v': "4.2.0",
    ...
    if (__WEBPACK_IMPORTED_MODULE_3__src_config_js__["b" /* config */].getConfig('userSync') && __WEBPACK_IMPORTED_MODULE_3__src_config_js__["b" /* config */].getConfig('userSync').syncsPerBidder) {
      payload.us = __WEBPACK_IMPORTED_MODULE_3__src_config_js__["b" /* config */].getConfig('userSync').syncsPerBidder;
    ...
    if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(validBidRequests[0], 'params.hfa')) {
      payload.hfa = Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(validBidRequests[0], 'params.hfa');
    }
    ...
    if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(validBidRequests[0], 'userId') && Object.keys(validBidRequests[0].userId).length > 0) {
      payload.userid = JSON.stringify(validBidRequests[0].userId);
    }

    var keywords = validBidRequests[0].params.keywords; // a CSV of keywords
    ...
    if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["isEmpty"])(data)) {
      return null;
    }

    var url = STR_ENDPOINT;

    if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(validBidRequests[0], 'params.bid_request_url')) {
      url = Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(validBidRequests[0], 'params.bid_request_url');
    }
  ...
  interpretResponse: function interpretResponse(serverResponse, bidderRequest) {
    ...
    Object.keys(bidResponse.slots).forEach(function (slot) {
      ...
      if (bid.sbi_ct === 'video') {
        mediaType = 'video';
        var context = Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bidRequest, 'mediaTypes.video.context');
      ...
      if (bid.sbi_aid && bid.sbi_mouse && bid.sbi_size) {
        ...
        if (mediaType === 'video') {
          ...
          bids.mediaType = 'video';
          bids.vastUrl = createCreative(bidResponse.sbi_dc, bid.sbi_aid);
          bids.renderer = newRenderer(bidRequest.adUnitCode, bids, Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bidRequest, 'renderer.options'));
          var videoSize = Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bidRequest, 'params.sizes');
...
function _validateSize(bid) {
  if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.video')) {
    return ''; // Video bids arent allowed to override sizes via the trinity request
  }

  if (bid.params.sizes) {
    return Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["parseSizesInput"])(bid.params.sizes).join(',');
  }

  if (Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.banner.sizes')) {
    return Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["parseSizesInput"])(Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["deepAccess"])(bid, 'mediaTypes.banner.sizes')).join(',');
  } // Handle deprecated sizes definition


  if (bid.sizes) {
    return Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["parseSizesInput"])(bid.sizes).join(',');
  }
}
...
function newRenderer(adUnitCode, bid) {
  ...
  try {
    ...
    Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logWarn"])('Prebid Error calling setRender on renderer', err);
  }

  renderer.setEventHandlers({
    impression: function impression() {
      return Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logMessage"])('Sonobi outstream video impression event');
    },
    loaded: function loaded() {
      return Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logMessage"])('Sonobi outstream video loaded event');
    },
    ended: function ended() {
      Object(__WEBPACK_IMPORTED_MODULE_1__src_utils_js__["logMessage"])('Sonobi outstream renderer video event'); // document.querySelector(`#${adUnitCode}`).style.display = 'none';
    }
  });
  return renderer;
}
...
function _iframeAllowed() {
  return __WEBPACK_IMPORTED_MODULE_5__src_userSync_js__["a" /* userSync */].canBidderRegisterSync('iframe', BIDDER_CODE);
}
